...
def filter_mnist(dataset, digits):
  def filter_fn(x):
    return tf.reduce_any(tf.equal(x['label'], digits))
  return dataset.filter(filter_fn)

def load_filtered_mnist(digits):
  ...
  filtered_train_dataset = filter_mnist(train_dataset, digits)
  filtered_test_dataset = filter_mnist(test_dataset, digits)

  return filtered_train_dataset, filtered_test_dataset, dataset_builder
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  if config.data.dataset == 'CIFAR10':
    ...
    digits_to_include = [0, 1, 2, 3, 4]
    train_dataset, test_dataset, dataset_builder = load_filtered_mnist(digits_to_include)
    train_split_name = 'train'
    eval_split_name = 'test'
  ...
  def create_dataset(ds, split):
    ...
    if isinstance(ds, tfds.core.DatasetBuilder):
  ...
  if config.data.dataset == 'MNIST':
    train_ds = create_dataset(train_dataset, train_split_name)
  else:
    train_ds = create_dataset(dataset_builder, train_split_name)
  print("building test")
  if config.data.dataset == 'MNIST':
    eval_ds = create_dataset(test_dataset, eval_split_name)
  else:
